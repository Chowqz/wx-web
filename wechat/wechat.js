        ...
        if(accessTokenJson.access_token === "" || accessTokenJson.expires_time < currentTime){
            that.requestGet(url).then(function(data){
                ...
                if(data.indexOf("errcode") < 0){
                    ...
                    //更新本地存储的
                    fs.writeFile('./wechat/access_token.json',JSON.stringify(accessTokenJson), function(err, data) {
                        if (err) {
                            throw err;
                        }
                        console.log(data);
...
WeChat.prototype.getJsapiTicket = function(){
    ...
    return new Promise(function(resolve,reject){
        ...
        var currentTime = new Date().getTime();
        //格式化请求地址
        var url = util.format(that.apiURL.jsapi_ticketApi, that.apiDomain, accessTokenJson.access_token);
        ...
        if(jsapiTicketJson.jsapi_ticket === "" || jsapiTicketJson.expires_time < currentTime){
            that.requestGet(url).then(function(data){
                ...
                if(result.errcode == 0){
                    ...
                    //更新本地存储的
                    fs.writeFile('./wechat/jsapi_ticket.json',JSON.stringify(jsapiTicketJson), function(err, data) {
                        if (err) {
                            throw err;
                        }
                        console.log(data);
